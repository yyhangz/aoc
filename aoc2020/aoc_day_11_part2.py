...
def num_occ(row, col):
    ...
    for i in range(-1, 2):
        for j in range(-1, 2):
            if i != 0 or j != 0:
                k = 1
                while(within_seats(row + k*i, col + k*j)):
                    if prev[row + k*i][col + k*j] == '#':
                        count += 1
                        break
                    elif prev[row + k*i][col + k*j] == 'L':
                        break
                    k += 1
    return count

def within_seats(row, col):
    return 0 <= row < len(prev) and 0 <= col < len(prev[0])
